#!/usr/bin/env python3
"""
Import de l'indice ATMO annuel pour Lyon dans PostgreSQL.

Source : Atmo Auvergne-Rhône-Alpes (https://www.atmo-auvergnerhonealpes.fr/)
À téléverser dans scripts/data/atmo-lyon.json au format normalisé suivant :
...
DATA_FILE = Path(__file__).parent / "data" / "atmo-lyon.json"
SOURCE_LABEL = "Atmo Auvergne-Rhône-Alpes — indice ATMO annuel"
VILLE = "lyon"
...
def load_data():
    if not DATA_FILE.exists():
        ...
        print(f"Placer le bilan ATMO Lyon dans ce fichier puis relancer le script.")
        sys.exit(1)
...
def normalize_entry(entry):
    ...
    Adapter ici si Atmo AuRA utilise d'autres noms de champs."""
    return {
